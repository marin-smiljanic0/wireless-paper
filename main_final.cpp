...
int main(int argc, char **argv) {
    ...
    std::vector<double> userRatios = {1, 2, 4, 8};

    for (int n_relays = 1; n_relays <= 10; ++n_relays) {
	for (int chunks = 4; chunks <= n; chunks *= 2) {
		...
		for (int j = 1; j <= 20; ++j) {
		    double best = 0.0;
		    double best_ber = 1000000.0;
		    for (int k = 0; k < n_relays; ++k) {
			//std::cout << chunks << " " << i*100 + j << std::endl;
			std::ostringstream s;
			s << "test_data_new_large/kanal" << (rand() % 100 + 1) << ".txt";
			std::string s1 = s.str();
			FILE *fin = fopen(s1.c_str(), "r");

			std::ostringstream s2;
			s2 << "test_data_new_large/kanal" << (rand() % 100 + 1) << ".txt";
	    ...
	    printf("(%d %.2lf %.9lf) ", chunks, 1.0 * sum / 400, ber_sum / 400);
	}
